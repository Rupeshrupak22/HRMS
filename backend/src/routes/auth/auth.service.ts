...
export async function login(dto: LoginDto & { forceLogin?: boolean; deviceId?: string }) {
  ...
  if (activeSession && !dto.forceLogin) {
  ...
  const tokens = generateTokens(user.id, user.email, user.role, deviceId);
 ...
 */
function generateTokens(userId: string, email: string, role: string, deviceId?: string) {
  const payload: JwtPayload & { deviceId?: string } = { sub: userId, email, role };
