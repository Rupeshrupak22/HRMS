...
export default function OverallReportPage() {
    ...
    async function load() {
      try {
        const [ret, res, ex, fnf, comp, intv, aDr,
               perf, disc, rel, nDr,
               onb, drop, vDr,
               payroll, dailyAll] = await Promise.all([
  ...
  const rd = rawData || { ret: [], res: [], ex: [], fnf: [], comp: [], intv: [], aDr: [], perf: [], disc: [], rel: [], nDr: [], onb: [], drop: [], vDr: [], payroll: [], dailyAll: [] };

  const charithaDaily = fd(rd.dailyAll).filter((r: any) => r.userEmail === 'charitha@adyapan.com' || r.specialization === 'SALARY_PAYROLL').length;
  const pavitraDaily = fd(rd.dailyAll).filter((r: any) => r.userEmail === 'pavitra@adyapan.com' || r.specialization === 'ATTENDANCE_LEAVE').length;
  ...
  const data = {
    ...
    charitha: {
      ...
      dailyReports: charithaDaily > 0 ? charithaDaily : fd(rd.payroll).length,
    },
    pavitra: {
      dailyReports: pavitraDaily,
      totalRecords: pavitraDaily,
    },
  };
  ...
  const handleSubmitToAdmin = async () => {
    ...
    try {
      const report = {
        ...
        pavitraSummary: `DailyReports:${data.pavitra.dailyReports}`,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-2xl bg-white border border-slate-200 shadow-xs">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-slate-800">Filter Performance Date:</span>
          {filterDate && (
        ...
        <div className="flex items-center gap-3">
          <input
            ...
            className="px-3 py-1.5 rounded-xl border border-slate-300 text-xs font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-orange-500 cursor-pointer"
          />
          {filterDate && (
            <button
              onClick={() => setFilterDate('')}
              className="px-3 py-1.5 rounded-xl bg-slate-100 text-xs font-bold text-slate-600 hover:bg-slate-200 cursor-pointer"
            >
              Show All Time
            </button>
          )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-xs">
          ...
          <div className="text-[10px] text-slate-500 mt-1">{filterDate ? `For ${filterDate}` : 'Across all modules'}</div>
        </div>
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-xs">
      ...
      <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm">
        <h2 className="text-sm font-bold text-slate-900 border-b border-slate-100 pb-3 mb-4 flex items-center justify-between">
          <span>Specialist-wise Performance Summary</span>
          {filterDate && <span className="text-xs text-slate-500 font-normal">Date Filter: <strong className="text-slate-800">{filterDate}</strong></span>}
        </h2>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              ...
              <tr className="border-b border-slate-100 hover:bg-orange-50/30">
                ...
                <td className="px-4 py-3 text-slate-700">
                  Attendance logs & leave approvals recorded
                </td>
