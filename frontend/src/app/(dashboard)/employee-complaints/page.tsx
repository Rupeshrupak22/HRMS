...
import { MessageSquareWarning, Plus, X, Pencil, Trash2 } from 'lucide-react';
...
const initialData: ComplaintRecord[] = [
  {
    ...
    manager: 'Karan Patel',
    category: 'Harassment',
    complaintSummary: 'Verbal harassment by team lead during stand-up meetings',
    assignedTo: 'Nitisha (POSH)',
  ...
  },
  {
    ...
    complaintSummary: 'Unfair workload distribution and denied leave',
    assignedTo: 'Aravind (Exit)',
    actionTaken: 'Mediation session conducted',
    status: 'Resolved',
  },
];
...
export default function EmployeeComplaintsPage() {
  ...
  const [form, setForm] = useState<Omit<ComplaintRecord, 'id'>>({
    ...
    category: '', complaintSummary: '', assignedTo: '', actionTaken: '', status: '',
  });

  const resetForm = () => {
    setForm({ employeeId: '', name: '', department: '', dateReceived: '', manager: '',
      category: '', complaintSummary: '', assignedTo: '', actionTaken: '', status: '' });
    setEditingId(null);
    setShowForm(false);
  };
  ...
  useEffect(() => {
    aravindApi.getComplaints().then(setRecords).catch(() => {});
  }, []);
            ...
            <tbody>
              {records.map((r) => (
                <tr key={r.id} className="border-b border-slate-100 hover:bg-orange-50/30">
                  ...
                  <td className="px-3 py-3 text-slate-700 max-w-[200px] truncate">{r.complaintSummary}</td>
