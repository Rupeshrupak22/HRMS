...
import { Users, Building2, DollarSign, FileText, Send, Loader2, CheckCircle2, Clock } from 'lucide-react';
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        console.error('Failed to load dashboard report counts:', err);
      } finally {
        setLoading(false);
      }
    }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-xs flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl font-black text-emerald-600 mt-1">
              {loading ? (
                <span className="inline-flex items-center gap-2 text-base text-slate-500 font-bold">
                  <Loader2 className="w-4 h-4 animate-spin text-emerald-600" /> Loading...
                </span>
              ) : (
                `${totalReportsToday} Submitted`
              )}
            </div>
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-xs flex items-center justify-between">
          <div>
            ...
            <div className={`text-2xl font-black mt-1 ${pendingReports > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
              {loading ? (
                <span className="inline-flex items-center gap-2 text-base text-slate-500 font-bold">
                  <Loader2 className="w-4 h-4 animate-spin text-slate-400" /> Loading...
                </span>
              ) : pendingReports > 0 ? (
              ...
              {hrTeamMembers.map((member, i) => (
                <tr key={i} className="hover:bg-orange-50/40 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    {loading ? (
                      <span className="px-2.5 py-1 bg-slate-100 text-slate-600 border border-slate-200 text-[10px] font-bold rounded flex items-center gap-1.5 w-fit">
                        <Loader2 className="w-3 h-3 animate-spin text-slate-500" />
                        <span>LOADING...</span>
                      </span>
                    ) : member.reported ? (
                      <span className="px-2.5 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 text-[10px] font-bold rounded flex items-center gap-1 w-fit">
                        <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                        <span>SUBMITTED TODAY</span>
                      </span>
                    ) : (
                      <span className="px-2.5 py-1 bg-amber-50 text-amber-700 border border-amber-200 text-[10px] font-bold rounded flex items-center gap-1 w-fit">
